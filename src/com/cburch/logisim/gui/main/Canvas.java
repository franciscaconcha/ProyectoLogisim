...
public class Canvas extends JPanel implements LocaleListener, CanvasPaneContents {
	...
	private class MyListener implements MouseInputListener, KeyListener, PopupMenuListener, PropertyChangeListener {
	...
	private class MyProjectListener implements ProjectListener, LibraryListener, CircuitListener, AttributeListener,
		...
		public void propagationCompleted( SimulatorEvent e ) {
			/*
			 * This was a good idea for a while... but it leads to problems when a
			 * repaint is done just before a user action takes place. // repaint -
			 * but only if it's been a while since the last one long now =
			 * System.currentTimeMillis(); if (now > lastRepaint + repaintDuration)
			 * { lastRepaint = now; // (ensure that multiple requests aren't made
			 * repaintDuration = 15 + (int) (20 * Math.random()); //
			 * repaintDuration is for jittering the repaints to // reduce aliasing
			 * effects repaint(); }
			 */
			paintThread.requestRepaint();
		}
	...
	private class MyViewport extends JViewport {
		...
		void paintContents( Graphics g ) {
			 ...
			 * >= 10000000) { speedStr = (speed / 1000000) + " MHz"; } else if
			 * (speed >= 1000000) { speedStr = (speed / 100000) / 10.0 + " MHz"; }
			 * else if (speed >= 10000) { speedStr = (speed / 1000) + " KHz"; }
			 * else if (speed >= 10000) { speedStr = (speed / 100) / 10.0 + " KHz";
			 * } else { speedStr = speed + " Hz"; } FontMetrics fm =
			 * g.getFontMetrics(); g.drawString(speedStr, getWidth() - 10 -
			...
			if( isNorth ) GraphicsUtil.drawArrow( g, sz.width / 2, 20, sz.width / 2, 2, 10, 30 );
			if( isSouth ) GraphicsUtil.drawArrow( g, sz.width / 2, sz.height - 20, sz.width / 2, sz.height - 2, 10, 30 );
			if( isEast ) GraphicsUtil.drawArrow( g, sz.width - 20, sz.height / 2, sz.width - 2, sz.height / 2, 10, 30 );
			if( isWest ) GraphicsUtil.drawArrow( g, 20, sz.height / 2, 2, sz.height / 2, 10, 30 );
			if( isNortheast ) GraphicsUtil.drawArrow( g, sz.width - 14, 14, sz.width - 2, 2, 10, 30 );
			if( isNorthwest ) GraphicsUtil.drawArrow( g, 14, 14, 2, 2, 10, 30 );
			if( isSoutheast ) GraphicsUtil.drawArrow( g, sz.width - 14, sz.height - 14, sz.width - 2, sz.height - 2, 10,
			      30 );
			if( isSouthwest ) GraphicsUtil.drawArrow( g, 14, sz.height - 14, 2, sz.height - 2, 10, 30 );
