...
public void representationMatrixTest() throws InvalidTransitionException, AbsentStateException{
	...
	sd.addState(s3.getName(), s3.getId());
	t1=new Transition(s1, s2, "1", "0");
	t2=new Transition(s2, s3, "0", "*");
	t3=new Transition(s3, s1, "*", "1");
	String[][][] mat=new String[3][3][2];
	mat[0][1][0]=t1.getInput();
	mat[0][1][1]=t1.getOutput();
	mat[1][2][0]=t2.getInput();
	mat[1][2][1]=t2.getOutput();
	mat[2][0][0]=t3.getInput();
	mat[2][0][1]=t3.getOutput();
	...
	assertEquals(3, repMat.getSize());
	assertEquals(mat[0][1][0], repMat.getInput(0, 1));
	assertEquals(mat[0][1][1], repMat.getOutput(0, 1));
	assertEquals(mat[1][2][0], repMat.getInput(1, 2));
	assertEquals(mat[1][2][1], repMat.getOutput(1, 2));
	assertEquals(mat[2][0][0], repMat.getInput(2, 0));
	assertEquals(mat[2][0][1], repMat.getOutput(2, 0));
	String[][][] sMat=repMat.getMatrix();
	assertEquals(mat[0][1][0], sMat[0][1][0]);
	assertEquals(mat[0][1][0], sMat[0][1][0]);
	assertEquals(mat[1][2][0], sMat[1][2][0]);
	assertEquals(mat[1][2][1], sMat[1][2][1]);
	assertEquals(mat[2][0][0], sMat[2][0][0]);
	assertEquals(mat[2][0][1], sMat[2][0][1]);
